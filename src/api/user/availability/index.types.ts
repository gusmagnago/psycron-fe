...
export interface IUpdateCancellationRecoveryStatusPayload {
	recoveryStatus: Extract<
		CancellationRecoveryStatus,
		'ARCHIVED' | 'FOLLOWED_UP'
	>;
	slotId: string;
	therapistId: string;
}

export interface IUpdateCancellationRecoveryStatusResponse {
	message: string;
	slot: {
		_id: string;
