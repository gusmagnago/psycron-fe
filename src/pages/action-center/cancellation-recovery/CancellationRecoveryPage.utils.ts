...
export const isRecoveryStateResolved = (
			...
			(date.slots ?? [])
				...
				.map((slot) => {
					...
					return {
						...
						patientName: patientDetails.patientFullName,
	...
	rows.filter((row) => {
		...
		const matchesStatus =
			...
			(filters.status === 'resolved' &&
				isRecoveryStateResolved(row.recoveryState)) ||
