...
export const AvailabilityWorkspaceShell = ({
	actions,
	children,
	...
	const focusToggle = useCallback(
		(side: AvailabilityWorkspacePanelSide): void => {
			const toggle = side === 'left' ? leftToggleRef.current : rightToggleRef.current;
			toggle?.focus();
		},
		[]
	);
		...
		>
			...
			>
				...
				>
					...
					/>
					{viewbar}
					<WorkspaceMainContent contentMode={contentMode}>
						{isLoading ? (
							<WorkspaceLoaderRegion>
								<Loader />
							</WorkspaceLoaderRegion>
